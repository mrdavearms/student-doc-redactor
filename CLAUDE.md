...
**"The Mac build ships only a `.dmg` and electron-updater needs a `.zip`, so Mac users never get updates" is a FALSE ALARM — it has been raised and investigated before (September 2026, v1.9.2).** It reads correctly off the library: `MacUpdater.doDownloadUpdate()` throws `ERR_UPDATER_ZIP_FILE_NOT_FOUND` when the manifest has no `.zip`. But that method only runs when electron-updater downloads, and on macOS it never does: `autoDownload` is `false`, nothing in `main.cjs` calls `autoUpdater.downloadUpdate()`, and the Mac branch of `restart-and-install` returns before `quitAndInstall()`. `update-available` (detection) fires fine from a dmg-only `latest-mac.yml`. Checked then: the live manifest listed the arm64 `.dmg` with a base64 SHA-512, and `verify:mac-updater` passed against it. **Do not "fix" it by adding a `zip` target** — see `pickMacAsset` below for why a zip must not quietly become the install path, and it would not unblock Squirrel.Mac anyway (the signature is the blocker). The only real gap: copies older than v1.7.0 can't self-update and need one manual `.dmg` download.
...
- **Dependency advisories**: `npm audit` is at **0 vulnerabilities** as of 11 August 2026. The last one — `electron` GHSA-9f4c-93c8-jc8g — was closed by moving the runtime 40.10.6 → **43.3.0** per `docs/plans/2026-08-08-electron-major-upgrade.md`, which is now done and records what was verified. The advisory was never exploitable here (no iframes/webview, local-file loading only, `contextIsolation` on, popups and off-app navigation already denied); the real driver was that Electron 40 is off the supported line and receives no Chromium fixes. **Still do NOT run `npm audit fix --force`** — it makes runtime-major decisions as a side effect with no test pass.
